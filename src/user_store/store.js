...
export default new Vuex.Store({
    ...
    mutations:{
        SET_TOKEN(state, payload){
            let jwtPayload = JSON.parse(Buffer.from(payload.accessToken.split('.')[1], 'base64'));
            ...
            state.token.accessTokenExp = jwtPayload['exp'];
            state.token.refreshToken = payload.refreshToken;
        },
        LOGOUT(state){
            state.user = null;
            state.token.accessToken = null;
            state.token.accessTokenExp = null;
    ...
    actions:{
        ...
        reissue({commit}, refreshToken) {
            return AuthApi.reissue(refreshToken).then((response) => {
                commit('SET_TOKEN', response.data);
